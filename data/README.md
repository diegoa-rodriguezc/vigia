...
| `kb_docs/` | **Documentos no estructurados** (PDF/Word) que el RAG indexa y cita por página | **Sí** — con integridad verificable por `make kb-docs` (ver abajo) |
...
El documento incluido (*Política de Seguridad, Defensa y Convivencia Ciudadana 2022-2026*) es la
**copia íntegra** del documento oficial publicado por la Policía Nacional, versionada con cita —al
amparo de la reproducción de textos oficiales (art. 41, Ley 23 de 1982)— para que el pilar de datos no
estructurados sea reproducible en un clon. `make kb-docs` (incluido en `make deploy`) **verifica su
integridad por SHA-256** contra el original en línea y lo re-obtiene si faltara — hash e instrucciones
en [kb_docs/README.md](kb_docs/README.md), procedencia completa en
[docs/DATASETS.md](../docs/DATASETS.md#recursos-complementarios-fuera-de-soda2--procedencia-y-atribución).
Sin el documento, el índice del RAG degrada con elegancia (solo data cards de cifras).
